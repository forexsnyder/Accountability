import React, { useState } from "react";
import "./SignUpForm.css";
import axios from "axios";
import { Link } from "react-router-dom";

function SignUpForm(props) {
  const [name, updateName] = useState("");
  const [dGoal1, updateDGoal1] = useState("");
  const [dGoal2, updateDGoal2] = useState("");
  const [dGoal3, updateDGoal3] = useState("");
  const [wGoal1, updateWGoal1] = useState("");
  const [mGoal1, updateMGoal1] = useState("");
  const [coach, updateCoach] = useState("recMaR6APIP9U10ih");

  const handleSubmit = async (e) => {
    e.preventDefault();
    const data = await axios.post(
      "https://api.airtable.com/v0/appiY6zW8rQRwC3qa/Clients",
      {
        fields: {
          Name: name,
          DailyGoal1: dGoal1,
          DailyGoal2: dGoal2,
          DailyGoal3: dGoal3,
          WeeklyGoal1: wGoal1,
          MonthlyGoal1: mGoal1,
          Coach: [coach],
        },
      },
      {
        headers: {
          Authorization: `Bearer ${process.env.REACT_APP_AIRTABLE_KEY}`,
          "Content-Type": "application/json",
        },
      }
    );
    ///needed to pass these using props. beause needed on both component pages.
    props.updateFetchClients(!props.fetchClients);
    updateDGoal1("");
    updateDGoal2("");
    updateDGoal3("");
    updateWGoal1("");
    updateMGoal1("");
    updateName("");
    updateCoach("");
  };

  //Create Form
  return (
    <div className="signup-page">
      <h2>Please Sign Up</h2>
      <form className="signup-form" onSubmit={handleSubmit}>
        <div className="box-div" >
        <div className= "label-div">
        <label className="title" htmlFor="Name">Name</label>
        <input
          className= "text-div"
          type="Text"
          id="Name"
          onChange={(e) => updateName(e.target.value)}
          value={name}
        />
        </div>
        <div className= "label-div">
        <label className="title" htmlFor="DailyGoal1">1st Daily Goal</label>
        <input
        className= "text-div"
          type="Text"
          id="DailyGoal1"
          onChange={(e) => updateDGoal1(e.target.value)}
          value={dGoal1}
        />
        </div>
        <div className= "label-div">
        <label className="title" htmlFor="DailyGoal2">2nd Daily Goal</label>
        <input
          className= "text-div"
          type="Text"
          id="DailyGoal2"
          onChange={(e) => updateDGoal2(e.target.value)}
          value={dGoal2}
        />
        </div>
        <div className= "label-div">
        <label className="title" htmlFor="DailyGoal3">3rd Daily Goal</label>
        <input
          className= "text-div"
          type="Text"
          id="DailyGoal3"
          onChange={(e) => updateDGoal3(e.target.value)}
          value={dGoal3}
        />
        </div > 
        <div className= "label-div">
        <label className="title" htmlFor="WeeklyGoal1">Weekly Goal</label>
        <input
        className= "text-div"
          type="Text"
          id="WeeklyGoal1"
          onChange={(e) => updateWGoal1(e.target.value)}
          value={wGoal1}
        />
        </div >
        <div className= "label-div">
        <label className="title" htmlFor="MonthlyGoal1">Monthly Goal</label>
        <input
        className= "text-div"
          type="Text"
          id="MonthlyGoal1"
          onChange={(e) => updateMGoal1(e.target.value)}
          value={mGoal1}
        />
        </div>
        <div className= "label-div" >
          <p className="title" >Select your coach</p>
          <select  className="menu" value={coach} onChange={(e) => updateCoach(e.target.value)}>
            <option className="menu" value="recMaR6APIP9U10ih">Jenni</option>
            <option className="menu" value="rechcqRKnyf0AFEC2">Jeff</option>
          </select>
        </div>
        </div>
        <input className="submit-button" type="submit" value="Finish sign up" />
      </form>
      <div>
      <Link to="/which">Login</Link>
      </div>
    </div>
  );
}

export default SignUpForm;
